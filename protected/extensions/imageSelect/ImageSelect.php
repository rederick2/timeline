...
class ImageSelect extends CWidget{
			...
			$('#div_image_select_".$this->id." a').file().choose(function(e, input) {
				...
				$('#div_image_select_".$this->id." form').ajaxSubmit({
					success : function(responseText){
						...
						$('input#UserForm_picture').attr('value', responseText);
					}
				});
			});
	...
	public function run(){
		...
		echo '<a id="btn_change_image" class="image-select-edit btn" style="position: absolute; left: 20%; top: 20px;">'.$this->text.'</a><form id="frm_img_select" action="'.$this->uploadUrl.'" method="POST" enctype="multipart/form-data"></form>';
